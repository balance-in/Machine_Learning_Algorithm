...
def MSE(x,Wih,Whj,t):
    ...
    return res / 2 
def Backward_propagation(X_train,t,Wih,Whj):
    ...
    for i in range(0,mid_num):
        for j in range(0,output_num):
            Whj[i][j] =  Whj[i][j] + eta * theta_j[j] * out_y[i]
    #theta_h
    ...
    for i in range(0,input_num):
        for j in range(0,mid_num):
            Wih[i][j] =  Wih[i][j] + eta * theta_h[j] * X_train[i]
    return Wih,Whj
def S_train(Wih,Whj):
    w_s = random.randint(0,2)
    w_i = random.randint(0,9)
    if w_s == 0:
        Wih,Whj = Backward_propagation(x_train_1[w_i],t1,Wih,Whj)
    if w_s == 1:
        Wih,Whj = Backward_propagation(x_train_2[w_i],t2,Wih,Whj)    
    if w_s == 2:
        Wih,Whj = Backward_propagation(x_train_3[w_i],t3,Wih,Whj)
    return Wih,Whj
def Backward_propagation_Batch(X_train,t,Wih,Whj,del_Wih,del_Whj):
    out_y,net_y,out_z,net_z = Forward_propagation(X_train,Wih,Whj)
    error = t - out_z
    #计算theta_j
    theta_j = np.zeros(output_num)
...
def B_train(x_train_1,x_train_2,x_train_3,Wih,Whj):
    ...
    for x in x_train_1:
        del_Wih,del_Whj = Backward_propagation_Batch(x,t1,Wih,Whj,del_Wih,del_Whj)
    for x in x_train_2:
        del_Wih,del_Whj = Backward_propagation_Batch(x,t2,Wih,Whj,del_Wih,del_Whj)    
    for x in x_train_3:
        del_Wih,del_Whj = Backward_propagation_Batch(x,t3,Wih,Whj,del_Wih,del_Whj)    
    return Wih + del_Wih,Whj+del_Whj
def error_all(Wih,Whj):
    jw_list = []
    for i in range(0,10):
...
# =============================================================================
# =============================================================================
# Wih_s = np.random.uniform(-0.5/mid_num,0.5/mid_num,(input_num,mid_num))
# Whj_s = np.random.uniform(-0.5/mid_num,0.5/mid_num,(mid_num,output_num))
# Wih_b = np.random.uniform(-0.5/mid_num,0.5/mid_num,(input_num,mid_num))
# Whj_b = np.random.uniform(-0.5/mid_num,0.5/mid_num,(mid_num,output_num))
Wih_s = np.random.randn(input_num,mid_num)
Whj_s = np.random.randn(mid_num,output_num)
Wih_b = np.random.randn(input_num,mid_num)
Whj_b = np.random.randn(mid_num,output_num)
# np.save('Wih',Wih)
# np.save('Whj',Whj)
# =============================================================================
# =============================================================================
...
while(1):
    if (count >= 4000):
        break
    jw_list = []
    Wih_s,Whj_s = S_train(Wih_s,Whj_s)
    S.append(error_all(Wih_s,Whj_s))
    Wih_b,Whj_b = B_train(x_train_1,x_train_2,x_train_3,Wih_b,Whj_b)
    B.append(error_all(Wih_b,Whj_b))
    X.append(count)
    count = count + 1
plt.figure()    
plt.plot(X, S,label='Stochastic_Backpropagation')
plt.plot(X, B,label='Batch_Backpropagation')
